...
export const GlobalProductSearch = ({ onClose, onSelect, open }: GlobalProductSearchProps) => {
  ...
  const message = searchQuery.error instanceof Error ? searchQuery.error.message : t("search.error");
  ...
  return (
    <Dialog description={t("search.desc")} onClose={close} open={open} title={t("search.title")}>
      ...
      <div aria-live="polite" className="mt-4">
        {!canSearch ? <p className="rounded-lg bg-[var(--canvas)] px-3 py-2.5 text-sm text-[var(--muted)]">{t("search.hint")}</p> : null}
        {canSearch && searchQuery.isPending ? <p className="flex items-center gap-2 px-1 py-3 text-sm text-[var(--muted)]"><LoaderCircle className="animate-spin" size={16} /> {t("search.searching")}</p> : null}
        ...
        {canSearch && searchQuery.isSuccess && products.length === 0 ? <p className="rounded-lg bg-[var(--canvas)] px-3 py-2.5 text-sm text-[var(--muted)]">{t("search.empty")}</p> : null}
        {canSearch && searchQuery.isSuccess && products.length > 0 ? (
          <ul aria-label={t("search.results")} className="divide-y divide-[var(--line)] overflow-hidden rounded-xl bg-[var(--surface)] shadow-[var(--shadow-border)]">
            {products.map((product) => (
              <li key={product.id}>
                <button className="flex min-h-14 w-full items-center gap-3 px-3 py-2.5 text-left transition-[background-color] duration-150 hover:bg-[var(--canvas)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[var(--olive)]" onClick={() => selectProduct(product)} type="button">
                  ...
                  <span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold text-[var(--ink)]">{product.name}</span><span className="mt-0.5 block truncate text-sm text-[var(--muted)]">{product.sku}{product.category_name ? ` - ${product.category_name}` : ""}</span></span>
                  <Badge tone={product.is_active ? "success" : "neutral"}>{product.is_active ? t("common.active") : t("common.archived")}</Badge>
                </button>
              </li>
            ))}
          </ul>
