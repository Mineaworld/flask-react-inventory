...
const AccessDenied = () => (
  <div className="grid min-h-[60vh] place-items-center">
    <section className="surface-panel max-w-lg p-8 text-center">
      ...
      <h1 className="mt-4 text-balance font-display text-2xl font-semibold leading-tight tracking-[-0.04em] text-[var(--ink)]">Purchase access is restricted</h1>
      <p className="mx-auto mt-2 max-w-sm text-pretty text-sm leading-6 text-[var(--muted)]">Purchases are available to Admin and Manager accounts. Sales remain available.</p>
    </section>
  </div>
);
...
const DetailDialog = ({ kind, onClose, order }: DetailDialogProps) => {
  ...
  return (
    <Dialog description="" onClose={onClose} open title={`${kind === "purchase" ? "Purchase" : "Sale"} ${order.document_number}`}>
      {detailQuery.isPending ? <div aria-label="Loading document detail" className="space-y-3"><div className="h-20 animate-pulse rounded-xl bg-[var(--canvas)]" /><div className="h-28 animate-pulse rounded-xl bg-[var(--canvas)]" /></div> : null}
      {detailQuery.isError ? <ResourceError message={apiErrorMessage(detailQuery.error, "Document detail could not be loaded.")} onRetry={() => void detailQuery.refetch()} /> : detail ? (
        <div className="space-y-5">
          <dl className="grid grid-cols-2 gap-4 rounded-xl bg-[var(--canvas)] p-4 text-sm shadow-[inset_0_0_0_1px_var(--line)]">
            <div><dt className="text-xs uppercase tracking-wide text-[var(--muted)]">Partner</dt><dd className="mt-1 font-semibold text-[var(--ink)]">{kind === "purchase" ? (detail as Purchase).supplier_name : (detail as Sale).customer_name || "Walk-in"}</dd></div>
            <div><dt className="text-xs uppercase tracking-wide text-[var(--muted)]">State</dt><dd className="mt-1"><Badge tone={statusTone(detail.status)}>{detail.status}</Badge></dd></div>
            <div><dt className="text-xs uppercase tracking-wide text-[var(--muted)]">Document total</dt><dd className="mt-1 font-bold tabular-nums text-[var(--ink)]">{displayAmount(detail)}</dd></div>
            <div><dt className="text-xs uppercase tracking-wide text-[var(--muted)]">USD total</dt><dd className="mt-1 font-bold tabular-nums text-[var(--olive-strong)]">{formatCurrency(detail.total_usd, "USD")}</dd></div>
            <div><dt className="text-xs uppercase tracking-wide text-[var(--muted)]">Exchange rate</dt><dd className="mt-1 tabular-nums text-[var(--ink)]">{formatExchangeRate(detail.exchange_rate_to_usd)} {detail.currency}/USD</dd></div>
            <div><dt className="text-xs uppercase tracking-wide text-[var(--muted)]">Created</dt><dd className="mt-1 text-[var(--ink)]">{formatDate(detail.created_at)}</dd></div>
          </dl>
          <div>
            <h3 className="font-display text-base font-bold text-[var(--ink)]">Items</h3>
            <div className="mt-2 overflow-x-auto rounded-xl shadow-[inset_0_0_0_1px_var(--line)]">
              <table className="min-w-full text-left text-sm">
                <thead className="bg-[var(--canvas)] text-xs uppercase tracking-wide text-[var(--muted)]"><tr><th className="px-3 py-2">Product</th><th className="px-3 py-2 text-right">Quantity</th><th className="px-3 py-2 text-right">Unit</th><th className="px-3 py-2 text-right">Unit USD</th><th className="px-3 py-2 text-right">Line USD</th></tr></thead>
...
const ConfirmActionDialog = ({ action, kind, onClose, order }: ConfirmActionDialogProps) => {
  ...
  const title = action === "receive" ? "Receive purchase" : action === "complete" ? "Complete sale" : `Cancel ${kind}`;
  ...
  const description = action === "receive"
    ? "Receive and write stock movements."
    : action === "complete"
      ? "Deduct stock and complete sale."
      : `Cancel ${kind}.`;
  return (
    <Dialog description={description} onClose={onClose} open title={title}>
      {error ? <div className="mb-4 rounded-xl bg-[var(--coral-soft)] p-3 shadow-[inset_0_0_0_1px_var(--coral)]" role="alert"><p className="font-semibold text-[var(--coral-strong)]">{apiErrorMessage(error, "The action could not be completed.")}</p>{insufficientStock ? <p className="mt-1 text-xs text-[var(--coral-strong)]">Stock was not changed. Receive or adjust stock, then try again.</p> : null}</div> : null}
      <div className="rounded-xl bg-[var(--canvas)] p-3 text-sm shadow-[inset_0_0_0_1px_var(--line)]"><p className="font-semibold text-[var(--ink)]">{order.document_number}</p><p className="mt-1 tabular-nums text-[var(--muted)]">{displayAmount(order)} · USD total {formatCurrency(order.total_usd, "USD")}</p></div>
      <div className="mt-5 flex justify-end gap-3"><Button disabled={mutation.isPending} variant="secondary" onClick={onClose}>Back</Button><Button disabled={mutation.isPending} variant={action === "cancel" ? "danger" : "primary"} onClick={() => void confirm()}>{mutation.isPending ? "Working..." : title}</Button></div>
    </Dialog>
  );
};
  ...
  return (
    <div className="space-y-5">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div><h1 className="page-title">{kind === "purchase" ? "Purchases" : "Sales"}</h1></div>
        <Button onClick={() => setEditTarget("create")}><FilePlus2 size={17} />New {singular}</Button>
      </header>
      {kind === "sale" && role === "staff" ? <div><Badge tone="olive">Your drafts only</Badge></div> : null}
      <section className="surface-panel overflow-hidden">
        <div className="surface-toolbar p-4"><div className="relative max-w-md"><Search aria-hidden="true" className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[var(--muted)]" size={17} /><Input aria-label={`Search ${resource}`} className="pl-10" type="search" value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); }} placeholder={`Search ${singular} number`} /></div></div>
        ...
        {ordersQuery.isError ? <div className="p-4"><ResourceError message={apiErrorMessage(ordersQuery.error, `${resource} could not be loaded.`)} onRetry={() => void ordersQuery.refetch()} /></div> : !ordersQuery.data ? null : ordersQuery.data.data.length === 0 ? (
          <div className="grid min-h-48 place-items-center p-8 text-center"><div><span className="mx-auto grid size-10 place-items-center rounded-lg bg-[var(--olive-soft)] text-[var(--olive-strong)]">{kind === "purchase" ? <Truck size={20} /> : <ShoppingCart size={20} />}</span><p className="mt-3 font-semibold text-[var(--ink)]">No {resource} match this view.</p></div></div>
        ) : (
          <DataTable>
            <thead><tr className="bg-[var(--canvas)] text-xs uppercase tracking-[0.07em] text-[var(--muted)]"><th className="px-4 py-3"><SortButton active={sort === "document_number"} direction={direction} onClick={() => changeSort("document_number")}>Document</SortButton></th><th className="px-4 py-3">{kind === "purchase" ? "Supplier" : "Customer"}</th><th className="px-4 py-3">State</th><th className="px-4 py-3 text-right">Currency total</th><th className="px-4 py-3 text-right"><SortButton active={sort === "total_usd"} direction={direction} onClick={() => changeSort("total_usd")}>USD total</SortButton></th><th className="px-4 py-3"><SortButton active={sort === "created_at"} direction={direction} onClick={() => changeSort("created_at")}>Created</SortButton></th><th className="px-4 py-3"><span className="sr-only">Actions</span></th></tr></thead>
            <tbody>{ordersQuery.data.data.map((order) => {
              const draft = order.status === "draft";
              const partnerName = kind === "purchase" ? (order as Purchase).supplier_name : (order as Sale).customer_name || "Walk-in";
              const actionItems = [
                { icon: <Eye size={14} />, label: "View details", onSelect: () => setDetailTarget(order) },
                ...(draft ? [{ icon: <Pencil size={14} />, label: "Edit", onSelect: () => setEditTarget(order) }] : []),
                ...(draft && kind === "purchase" ? [{ icon: <Truck size={14} />, label: "Receive", onSelect: () => setActionTarget({ action: "receive", order }), tone: "success" as const }] : []),
                ...(draft && kind === "sale" && manager ? [{ icon: <ShoppingCart size={14} />, label: "Complete", onSelect: () => setActionTarget({ action: "complete", order }), tone: "success" as const }] : []),
                ...(draft ? [{ icon: <XCircle size={14} />, label: "Cancel", onSelect: () => setActionTarget({ action: "cancel", order }), tone: "danger" as const }] : []),
              ];
              return (
                <tr className="data-row border-t border-[var(--line)]" key={order.id}>
                  <td className="px-4 py-3"><p className="font-mono text-xs font-bold text-[var(--ink)]">{order.document_number}</p><p className="mt-1 text-xs text-[var(--muted)]">{order.items.length} {order.items.length === 1 ? "line" : "lines"}</p></td>
                  ...
                  <td className="px-4 py-3 text-xs text-[var(--muted)]">{formatDate(order.created_at)}</td>
      ...
      <Dialog description="" onClose={() => setEditTarget(null)} open={editTarget !== null} title={editTarget === "create" ? `New ${singular}` : `Edit ${singular}`}>
