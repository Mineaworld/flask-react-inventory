...
const Field = ({ children, error, label }: { children: React.ReactNode; error?: string; label: string }) => <label className="flex min-w-0 flex-col gap-1.5 text-sm font-semibold text-[var(--ink)]"><span className="truncate">{label}</span>{children}{error ? <span className="text-sm font-medium text-[var(--coral-strong)]">{error}</span> : null}</label>;
