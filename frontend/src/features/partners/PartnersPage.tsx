...
const StaffCustomerPicker = () => {
  ...
  return <section className="surface-panel overflow-hidden"><div className="surface-toolbar p-4"><div className="flex items-center gap-2 text-sm font-semibold text-[var(--olive-strong)]"><ShieldCheck size={17} />Safe customer picker</div><div className="relative mt-3 max-w-md"><Search aria-hidden="true" className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[var(--muted)]" size={17} /><Input aria-label="Search sale customers" className="pl-10" type="search" value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); }} placeholder="Search customers" /></div></div>{customersQuery.data.data.length === 0 ? <EmptyState icon={<Users size={22} />} text="No active customers match this search." /> : <DataTable><thead><tr className="bg-[var(--canvas)] text-xs uppercase tracking-[0.07em] text-[var(--muted)]"><th className="px-4 py-3">Customer</th><th className="px-4 py-3">Reference</th></tr></thead><tbody>{customersQuery.data.data.map((customer) => <tr className="data-row border-t border-[var(--line)]" key={customer.id}><td className="px-4 py-3 font-semibold text-[var(--ink)]">{customer.name}</td><td className="px-4 py-3 font-mono text-xs text-[var(--muted)]">{customer.code}</td></tr>)}</tbody></DataTable>}<Pagination label="customers" meta={customersQuery.data.meta} onPageChange={setPage} onPerPageChange={(size) => { setPerPage(size); setPage(1); }} /></section>;
};
...
export const PartnersPage = ({ role }: PartnersPageProps) => {
  ...
  if (!isManager) return <div className="space-y-5"><header><h1 className="page-title">Customers</h1></header><StaffCustomerPicker /></div>;
  return (
    <div className="space-y-5">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6">
          ...
          <div aria-label="Partner sections" className="inline-flex rounded-xl bg-[var(--surface)] p-1 shadow-[var(--shadow-border)]">
            <button aria-pressed={tab === "customers"} className={`min-h-9 rounded-lg px-4 text-sm font-semibold transition-[background-color,color] duration-150 ${tab === "customers" ? "bg-[var(--ink)] text-[var(--on-ink)]" : "text-[var(--muted)] hover:bg-[var(--canvas)]"}`} type="button" onClick={() => { setTab("customers"); setPage(1); setQuery(""); }}>Customers</button>
            <button aria-pressed={tab === "suppliers"} className={`min-h-9 rounded-lg px-4 text-sm font-semibold transition-[background-color,color] duration-150 ${tab === "suppliers" ? "bg-[var(--ink)] text-[var(--on-ink)]" : "text-[var(--muted)] hover:bg-[var(--canvas)]"}`} type="button" onClick={() => { setTab("suppliers"); setPage(1); setQuery(""); }}>Suppliers</button>
          </div>
        </div>
        <Button onClick={() => setFormMode({ mode: "create", tab })}><Plus size={17} />New {partnerLabel(tab).toLowerCase()}</Button>
      </header>
      <section className="surface-panel overflow-hidden">
        <div className="surface-toolbar p-4"><div className="relative max-w-md"><Search aria-hidden="true" className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[var(--muted)]" size={17} /><Input aria-label={`Search ${tab}`} className="pl-10" type="search" value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); }} placeholder={`Search ${tab}`} /></div></div>
        ...
        {partnersQuery.isError ? <div className="p-4"><ResourceError message={apiErrorMessage(partnersQuery.error, `${partnerLabel(tab)}s could not be loaded.`)} onRetry={() => void partnersQuery.refetch()} /></div> : !partnersQuery.data ? null : partnersQuery.data.data.length === 0 ? <EmptyState icon={tab === "customers" ? <Users size={22} /> : <Truck size={22} />} text={`No ${tab} match this search.`} /> : <DataTable><thead><tr className="bg-[var(--canvas)] text-xs uppercase tracking-[0.08em] text-[var(--muted)]"><th className="px-4 py-3"><PartnerSortButton active={sort === "name"} direction={direction} onClick={() => changeSort("name")}>Name</PartnerSortButton></th><th className="px-4 py-3">Contact</th><th className="px-4 py-3">Phone</th><th className="px-4 py-3"><PartnerSortButton active={sort === "created_at"} direction={direction} onClick={() => changeSort("created_at")}>Created</PartnerSortButton></th><th className="px-4 py-3">Status</th><th className="px-4 py-3"><span className="sr-only">Actions</span></th></tr></thead><tbody>{partnersQuery.data.data.map((partner) => <tr className="data-row border-t border-[var(--line)]" key={partner.id}><td className="px-4 py-3"><p className="font-semibold text-[var(--ink)]">{partner.name}</p><p className="mt-0.5 text-xs text-[var(--muted)]">{partner.email || "No email"}</p></td><td className="px-4 py-3 text-[var(--muted)]">{partner.contact_name || "-"}</td><td className="px-4 py-3 tabular-nums text-[var(--muted)]">{partner.phone || "-"}</td><td className="px-4 py-3 text-xs text-[var(--muted)]">{formatDate(partner.created_at)}</td><td className="px-4 py-3"><Badge tone={partner.is_active ? "success" : "neutral"}>{partner.is_active ? "Active" : "Archived"}</Badge></td><td className="px-4 py-3 text-right"><ActionMenu triggerLabel={`More actions for ${partner.name}`} items={[{ icon: <Edit3 size={14} />, label: "Edit", onSelect: () => setFormMode({ mode: "edit", tab, partner }) }, ...(partner.is_active ? [{ label: "Archive", onSelect: () => { setArchiveError(null); setArchiveTarget(partner); }, tone: "danger" as const }] : [])]} /></td></tr>)}</tbody></DataTable>}
      ...
      <Dialog description="" onClose={() => setFormMode(null)} open={formMode !== null} title={formMode?.mode === "edit" ? `Edit ${partnerLabel(formMode.tab).toLowerCase()}` : `New ${formMode ? partnerLabel(formMode.tab).toLowerCase() : "partner"}`}>{formMode ? <PartnerForm formMode={formMode} onClose={() => setFormMode(null)} /> : null}</Dialog>
      <ConfirmArchiveDialog description="" error={archiveError} onClose={() => { setArchiveTarget(null); setArchiveError(null); }} onConfirm={() => void archive()} open={archiveTarget !== null} pending={archiveMutation.isPending} title={`Archive ${partnerLabel(tab).toLowerCase()}`} />
    </div>
  );
};
