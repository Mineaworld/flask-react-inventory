...
export const ResourceError = ({ message, onRetry, title = "Could not load this workspace" }: ResourceErrorProps) => (
  <section className="rounded-xl bg-[var(--coral-soft)] p-5 shadow-[inset_0_0_0_1px_var(--coral)]" role="alert">
    <h2 className="font-display text-lg font-semibold tracking-[-0.03em] text-[var(--ink)]">{title}</h2>
    <p className="mt-1 text-sm text-[var(--coral-strong)]">{message}</p>
    <Button className="mt-4" size="small" variant="secondary" onClick={onRetry}><RotateCcw size={15} />Try again</Button>
  </section>
);
...
export const Pagination = ({ label, meta, onPageChange, onPerPageChange }: PaginationProps) => {
  ...
  return (
    <footer className="flex flex-col gap-3 border-t border-[var(--line)] px-4 py-3.5 text-xs text-[var(--muted)] sm:flex-row sm:items-center sm:justify-between">
      <div className="flex items-center gap-3"><span className="tabular-nums">{start}-{end} of {meta.total} {label}</span><label className="flex items-center gap-1.5">Rows<select aria-label={`Rows per page for ${label}`} className="min-h-10 rounded-lg border border-[var(--line-strong)] bg-[var(--surface)] px-2 text-base text-[var(--ink)] sm:text-xs" value={meta.per_page} onChange={(event) => onPerPageChange(Number(event.target.value))}>{[10, 25, 50, 100].map((size) => <option key={size} value={size}>{size}</option>)}</select></label></div>
      <nav aria-label={`${label} pagination`} className="flex items-center gap-2">
        <Button aria-label="Previous page" disabled={meta.page <= 1} size="small" variant="secondary" onClick={() => onPageChange(meta.page - 1)}><ChevronLeft size={15} />Previous</Button>
        <span className="min-w-14 text-center font-semibold tabular-nums text-[var(--ink)]">{meta.page} / {pages}</span>
        <Button aria-label="Next page" disabled={meta.page >= pages || meta.pages === 0} size="small" variant="secondary" onClick={() => onPageChange(meta.page + 1)}>Next<ChevronRight size={15} /></Button>
      </nav>
    </footer>
  );
};
...
export const ConfirmArchiveDialog = ({ description, error, onClose, onConfirm, open, pending = false, title }: ConfirmArchiveDialogProps) => (
  <Dialog description={description} onClose={onClose} open={open} title={title}>
    ...
    <div className="flex justify-end gap-3"><Button disabled={pending} variant="secondary" onClick={onClose}>Cancel</Button><Button disabled={pending} variant="danger" onClick={onConfirm}>{pending ? "Archiving..." : "Archive"}</Button></div>
  </Dialog>
);
